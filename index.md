## ncepstのページ
### リンク

- GitHub : [https://github.com/ncepst](https://github.com/ncepst)  
- Qiita  : [https://qiita.com/ncepst](https://qiita.com/ncepst)

## リポジトリ紹介

### PyExcelTools

Pythonで、エクセルグラフの書式設定を指定して作成するモジュールです。  

分析データをエクセルへ出力した後、体裁調整後のエクセルグラフ出力までを、    
Pythonで自動化する際に汎用的に使えるモジュールです。  
任意引数が多量にありますが、繰り返し同じグラフを作成する際には便利です。  

[https://github.com/ncepst/PyExcelTools](https://github.com/ncepst/PyExcelTools)

## 記事
- [高周波特性1](RF_formula.md)
- [高周波特性2](RF_formula2.md)  
- [L型マッチング回路](L_matching.md)
- [シリアル通信](Serial_Comm.md)
- [MOSFETの基礎](mosfet1.md)
- [トランス](transformer.md)

## 経歴
～2022年： 修士 (電子工学・スピントロニクス)  
2022年～： 光センサーの開発エンジニア

## 自己紹介
社会人4年目。

学生時代から専門書を数式を理解しながら読むのが得意です。    
最近は生成AIも活用し、電子回路について個人学習中です。   

## お問い合わせ
ご連絡は下記メールにお願いします。  
E-mail: ncepst708 [at] gmail.com
